function SLAPmi_setPiezo (hSLAPMi)
if ~isempty(hSLAPMi) && ~strcmpi(hSLAPMi.status, 'idle')
    disp('SLAPmi should be idle to adjust Piezo parameters')
    return
end
...
%piezo input voltage is 0-10V, corresponging to 0-400um
...
%dummy check
if opts.Zmin<0 || opts.Zmin>10 || opts.Zmax<0 || opts.Zmax>10
   error('Requested illegal Piezo Z range'); 
end
