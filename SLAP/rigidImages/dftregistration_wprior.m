function [output Greg] = dftregistration_wprior(buf1ft,buf2ft,usfac, prior)
%prior: a factor that favors smaller motion offsets
%the prior strength is applied to the normalized error and should be in [0 1]
%the prior width is in pixels
...
elseif usfac == 1,
    ...
    P1 = [(0:floor(m/2-1)), (ceil(m/2-1):-1:0)]';
    P2 = [(0:floor(n/2-1)), (ceil(n/2-1):-1:0)];
    P = exp(-((repmat(P1.^2, 1, n) + repmat(P2.^2, m,1))./(2*prior.width.^2)));
    P = 1./(1 + ((1-exp(-1))./P).^prior.sharpness);
    P = 1-prior.strength  + prior.strength*P;
    CC = CC.*P;
    
    rfzero = sum(abs(buf1ft(:)).^2)/(m*n);
    rgzero = sum(abs(buf2ft(:)).^2)/(m*n); 
    error = 1.0 - CC.*conj(CC)/(rgzero(1,1)*rfzero(1,1));
    error = sqrt(abs(error));
...
% Partial-pixel shift
    ...
    P1 = [(0:floor(mlarge/2-1)), (ceil(mlarge/2-1):-1:0)]';
    P2 = [(0:floor(nlarge/2-1)), (ceil(nlarge/2-1):-1:0)];
    P = exp(-((repmat(P1.^2, 1, nlarge) + repmat(P2.^2, mlarge,1))./(2*(2*prior.width).^2)));
    P = 1./(1 + ((1-exp(-1))./P).^prior.sharpness);
    P = 1-prior.strength  + prior.strength*P;
    CC = CC.*P;
