function b = medfilt2_kp(varargin)
%%% ADDED FEATURE: padopt 'replicate'
...
if (rem(prod(mn), 2) == 1)
    ...
    if tf
        ...
        order = (prod(mn)+1)/2;
        b = ordfilt2_kp(a, order, domain, padopt);
    end
else
    order1 = prod(mn)/2;
    order2 = order1+1;
    b = ordfilt2_kp(a, order1, domain, padopt);
    b2 = ordfilt2_kp(a, order2, domain, padopt);
	if islogical(b)
		b = b | b2;
	else
...
elseif isempty(charLocation)
    ...
    options = {'indexed', 'zeros', 'symmetric', 'replicate'};
