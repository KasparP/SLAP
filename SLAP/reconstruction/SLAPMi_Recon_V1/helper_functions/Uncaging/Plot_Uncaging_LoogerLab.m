function Plot_Uncaging_LoogerLab(sys)
...
%     T= find(mDFF(i,tmin:tmax)>= minDFF(i) + 0.67*(maxDFF(i)-minDFF(i)),1,'first');
    T= find(mDFF(i,tmin:tmax)>= minDFF(i) + 0.5*(maxDFF(i)-minDFF(i)),1,'first');
    if ~isempty(T)
       maxDFF_time(i) = T;
    end
end
...
% R = cmin + (cmax-cmin)*max(0,min(1, (maxDFF_time-tmin)./(tmax-tmin))); %10ms->0.4 50ms->1
% R(R>=cmin) = 1+cmin-R(R>=cmin);
% R = R/2;
% 
% G = Svalid*(sys.output.F(:,1)+1);
% ff = max(G)/4; 
% G = min(1, (G./ff))/2;
% B = G;
% rgb_image = [R,G,B];
...
L = strcat({'Uncaging Location '}, int2str([1:length(uncagingx)]'));
L{3} = 'Time to reach half Max {\Delta}F/F_0';
legend(L);
ylabel('{\Delta}F/F_0','Interpreter', 'TeX')
xlabel('time (ms)')
...
legend(strcat({'Uncaging Location '}, int2str([1:length(uncagingx)]')));
ylabel('Normalized {\Delta}F/F_0','Interpreter', 'TeX')
xlabel('time (ms)')
...
cmp = myColorMap(length(selected)+2);
...
% cmap1 = (hsv(100));
cmaprange = tmin:tmax;
cmap1 = myColorMap(length(cmaprange));
ax1 = axes('pos', [2000 2000 1 1]);
% colormap(ax1,cmap1(100*cmin:100*cmax,:));

colormap(ax1,cmap1);
h_cb1 = colorbar(ax1, 'axislocation', 'in','fontsize',20);
set(h_cb1, 'pos', [0.83    0.10    0.01   0.83], 'tickdir', 'out', 'ticklength', 0, 'ticks', linspace(0, 1, 11), 'ticklabels', int2str(linspace(cmaprange(1),cmaprange(end),11)'), 'fontsize', 12)
h_cb1.Label.String = 'Red Level = Delay (ms)';
